...
def test_bake_and_run_tests(cookies):
    with bake_in_temp_dir(cookies) as result:
        assert result.project.isdir()
        run_inside_dir('pytest', str(result.project)) == -1
        print("test_bake_and_run_tests path", str(result.project))


# TODO (oct 2020): this will fail because we are using setuptools_scm
# def test_bake_and_run_tests_with_setup(cookies):
#     with bake_in_temp_dir(cookies) as result:
#         assert result.project.isdir()
#         run_inside_dir('python setup test', str(result.project)) == -1
#         print("test_bake_and_run_tests path", str(result.project))
